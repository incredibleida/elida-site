...
- Phone layout (≤900px): the hero stacks (title, Socrates, pill, then the eight covers as a static 4 × 2 grid — `site.js` skips the orbit and re-parents the pill); the manifesto runs down the middle via `display: contents` on `.mani-cta`; Selected work sits on `work-texture.webp`; the studio bio is Fira Mono 14px on an orange panel from the left edge; case heads order title → tags → description → credits (two columns) and the project overview moves above "Next project".
